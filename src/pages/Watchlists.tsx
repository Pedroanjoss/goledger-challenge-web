...
export function Watchlists() {
  ...
  const [availableShows, setAvailableShows] = useState<TvShow[]>([]); // Para o Dropdown
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      // Carrega as listas e as séries em paralelo para otimizar tempo
  ...
  const handleSave = async (watchlistData: Watchlist, selectedShowsKeys: string[], isEditing: boolean) => {
    try {
      if (isEditing) {
      ...
      showToast("Erro ao salvar Watchlist.", "error");
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setWatchlists(prev => prev.filter(w => w['@key'] !== watchlistToDelete['@key']));
  ...
  return (
    <Container className={styles.container}>
      <Box mb={4} display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap" gap={2}>
        <Box>
          <Typography variant="h3" color="secondary" className={styles.pageTitle}>Minhas Listas</Typography>
          <Typography variant="subtitle1" color="text.secondary">
            Agrupe suas séries favoritas para assistir depois.
          </Typography>
        </Box>
        <Button variant="contained" color="secondary" startIcon={<AddIcon />} onClick={() => handleOpenForm()} className={styles.addButton}>
          Criar Lista
        </Button>
      </Box>

      {loading ? (
        <Box className={styles.loaderContainer}><CircularProgress color="secondary" /></Box>
      ) : (
        <Grid container spacing={3}>
          {watchlists.length === 0 ? (
             <Grid size={{ xs: 12 }}>
               <Typography variant="body1" color="text.secondary" align="center" sx={{ mt: 2, p: 4, border: '1px dashed #475569', borderRadius: 2 }}>
            ...
            watchlists.map((watchlist) => (
              <Grid size={{ xs: 12, sm: 6, md: 4 }} key={watchlist['@key']}>
                <WatchlistCard watchlist={watchlist} onEdit={handleOpenForm} onDelete={confirmDelete} />
              </Grid>
            ))
          )}
        </Grid>
