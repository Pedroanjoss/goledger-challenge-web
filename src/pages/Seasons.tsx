...
export function Seasons() {
  ...
  const [selectedSeasonToEdit, setSelectedSeasonToEdit] =
  ...
  const [seasonForEpisodes, setSeasonForEpisodes] = useState<Season | null>(
    null,
  );
  ...
  const loadSeasons = async () => {
    ...
    try {
      ...
      const showResponse = await api.post("/query/search", {
        query: {
          selector: { "@assetType": "tvShows", title: decodedShowTitle },
        },
      });
      const shows =
        showResponse.data.result ||
        (Array.isArray(showResponse.data) ? showResponse.data : []);
      ...
      const seasonResponse = await api.post("/query/search", {
        query: {
          selector: { "@assetType": "seasons", "tvShow.@key": realTvShowKey },
        },
      });
  ...
  const handleSave = async (seasonData: Season, isEditing: boolean) => {
    try {
      ...
      const msg = getErrorMessage(error, "Erro ao salvar temporada.");
      showToast(msg, "error");
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setSeasons((prev) =>
        prev.filter((s) => s["@key"] !== seasonToDelete["@key"]),
      );
      setDeleteModalOpen(false);
    } catch (error) {
      const msg = getErrorMessage(
        error,
        "Erro ao excluir temporada. Remova os episódios antes.",
      );
      showToast(msg, "error");
    } finally {
  ...
  if (!decodedShowTitle)
    return (
      <Container>
        <Typography mt={5}>Série não encontrada.</Typography>
      </Container>
    );
  ...
  return (
    <Container className={styles.container}>
      ...
      <DeleteConfirmModal
        ...
        titleToDelete={
          seasonToDelete ? `Temporada ${seasonToDelete.number}` : ""
        }
