...
export function EpisodesListModal({
  open,
  onClose,
  season,
  showTitle,
  ...
  const handleSave = async (episodeData: Episode, isEditing: boolean) => {
    ...
    try {
      ...
      const msg = getErrorMessage(error, "Erro ao salvar episódio.");
      showToast(msg, "error");
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setEpisodes((prev) =>
        prev.filter((e) => e["@key"] !== episodeToDelete["@key"]),
      );
      setDeleteOpen(false);
    } catch (error) {
      const msg = getErrorMessage(error, "Erro ao excluir episódio.");
      showToast(msg, "error");
    } finally {
  ...
  return (
    ...
    >
      ...
      <DeleteConfirmModal
        ...
        titleToDelete={
          episodeToDelete
            ? `Episódio ${episodeToDelete.episodeNumber} - ${episodeToDelete.title}`
            : ""
        }
